...
MODULE_ID = 'e2e.us_sec_lock'


def us_view_sec_notification(driver:webdriver.Chrome)->int:
    '''
    3- The app displays a notification if water detected
    '''
    
    func_id = MODULE_ID + '.us_view_sec_notification'
    # go to living room
    ...
    mqtt_send_notification_simulation('SEC_LOCK')
    ...
    try:
        ...
        if header.text != 'Room: Living Room \n Device: Security Lock' or message.text != 'Lock Open':
            return 1
        alert.find_element(By.CSS_SELECTOR,'button').click()
    except:
        return 1
